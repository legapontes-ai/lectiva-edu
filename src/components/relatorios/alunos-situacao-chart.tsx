...
const CORES: Record<string, string> = {
  Ativo: "var(--chart-1)",
  Concluinte: "var(--success)",
  Inativo: "var(--muted-foreground)",
};

/**
 * Distribuição de alunos por situação acadêmica (pizza). Recebe dados já
 * agregados pelo servidor via props.
 */
export function AlunosSituacaoChart({
  distribuicao,
}: {
    ...
    <div className="h-64 w-full">
      <ResponsiveContainer width="100%" height="100%">
        <PieChart>
          ...
          >
            {distribuicao.map((d) => (
              <Cell key={d.nome} fill={CORES[d.nome] ?? "var(--chart-2)"} />
            ))}
          </Pie>
          <Tooltip
